...
### 技术决策
- [x] 新增 TD-013: 高亮渲染垂直对齐策略

### 修改文件
- `frontend/src/components/LogViewer.tsx`: 修复 `renderText` 函数中搜索高亮和图层高亮的 `hlY` 计算
...
// 搜索/图层高亮错误地使用 startY - lineHeight/2 (= y + 4)
const hlY = startY - lineHeight / 2;
```

### 修复方案
统一所有高亮背景起始位置为 `y`，确保视觉对齐。
