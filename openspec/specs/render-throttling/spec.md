...
## Purpose
定义前端渲染热路径的节流与缓存能力：滚动位置在面板切换/重挂载时保持（常驻逐帧看门狗，paint 前同帧拉回）、渲染依赖 props 引用稳定（React.memo 生效）、重复内容行的渲染结果跨行复用（有界 LRU）、以及调试模式下的前端帧率/内存可观测。
## Requirements
### Requirement: 滚动位置保持（常驻逐帧看门狗）

系统 SHALL 在 dockview 面板激活/失活导致滚动容器被外部归零（不触发 scroll 事件）时，将滚动位置恢复至真实位置；看门狗 SHALL 以**逐帧常驻**方式运行（rAF 循环在 paint 前执行，不因空闲停止），并通过同步滚动状态区分「用户滚到顶部」与「外部归零」，避免误干预与恢复延迟。

#### Scenario: 面板切换不跳回首行

- **WHEN** 用户在面板 A 滚动到第 5000 行后切换到面板 B 再切回面板 A
- **THEN** 面板 A 视口保持在原滚动位置
- **AND** 不出现跳回首行（DOM 滚动条被外部归零后，看门狗在同一帧 paint 前拉回，不产生可见的归零帧）

#### Scenario: 看门狗常驻逐帧检测

- **WHEN** 无滚动、无面板切换、无程序化跳转且滚动位置已稳定
- **THEN** 看门狗持续逐帧检测（rAF 循环保持运行），不因空闲停止，确保任意时刻的外部归零均在下一帧 paint 前被纠正

#### Scenario: 用户主动滚到顶部不被误干预

- **WHEN** 用户通过滚动事件主动将滚动条滚到顶部（真实位置为 0）
- **THEN** 滚动事件同步更新滚动状态（scrollStateRef 归 0），看门狗据此判定为「用户滚顶」而非外部归零，不拉回旧位置

#### Scenario: 程序化跳转到顶部不被误干预

- **WHEN** 执行 scrollToIndex 且目标位置为顶部（scrollTop=0）
- **THEN** 程序化定位同步滚动状态（scrollStateRef 归 0），看门狗识别其为程序化定位，不同帧拉回旧位置

#### Scenario: 外部归零不被延迟恢复

- **WHEN** dockview 归零滚动条之前或之后触发 scroll 事件（例如组件重挂/布局操作产生的 scroll 事件）
- **THEN** 看门狗不受该 scroll 事件影响，仍在归零后的下一帧 paint 前拉回真实位置（不因「近期有滚动事件」而延迟恢复）
