...
系统 SHALL 为所有"打开文件夹"用户入口（统一打开按钮/菜单、Ctrl+Shift+O 快捷键、命令面板命令）提供一致的对话框分流：远程路径选择器为默认主路径；仅当外部桌面壳注入原生对话框（探测为真）时使用原生对话框。

#### Scenario: 远程模式下按快捷键打开文件夹
- **WHEN** 后端以纯服务模式运行（无桌面壳，原生对话框探测为 false），用户按下 Ctrl+Shift+O
- **THEN** 前端弹出 RemotePathPicker 供浏览并选择服务器端目录

#### Scenario: 远程模式下从命令面板打开文件夹
- **WHEN** 后端以纯服务模式运行，用户在命令面板执行"打开文件夹"
- **THEN** 前端弹出 RemotePathPicker

#### Scenario: 桌面模式下打开文件夹
- **WHEN** 外部桌面壳注入原生对话框（探测为 true），用户触发任一"打开文件夹"入口
- **THEN** 弹出原生文件夹选择对话框，不出现 RemotePathPicker

### Requirement: 回退选择的语义一致性

打开文件夹编排 SHALL 区分"用户取消"与"原生对话框不可用"两种结果：取消时不产生任何副作用，探测为不可用时默认使用远程选择器。

#### Scenario: 桌面模式下用户取消
- **WHEN** 用户在外部壳注入的原生文件夹对话框中点击取消
