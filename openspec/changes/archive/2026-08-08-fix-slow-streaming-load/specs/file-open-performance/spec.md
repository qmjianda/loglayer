...
File Open Performance capability optimizes the file-open pipeline to eliminate multi-second "正在加载流式日志..." delays, based on root causes confirmed by cross-stack timing instrumentation: (1) log-level stats run 6 serial ripgrep subprocesses that contend with indexing on first open, (2) stats are redundantly triggered 2-3 times by duplicate WS signal registration and double fetchLogLevelStats calls, (3) cache-hit opens recompute the file hash on every open (3.3s on a 1.3GB file).
