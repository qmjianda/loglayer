...
Load Timing Observability capability instruments the file-open pipeline across backend (Python/FastAPI) and frontend (React/TS) with timestamped, switchable timing logs. It enables locating which stage of the open pipeline (REST request, mmap indexing, cache deserialization, pipeline processing, stats calculation, WS signal delivery, frontend state transition) consumes the observed multi-second load time, and serves as a durable performance watchdog.
