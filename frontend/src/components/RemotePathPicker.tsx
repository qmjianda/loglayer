...
const FolderIcon = () => (
  <svg width="16" height="16" viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg">
    <path
      d="M1 3.5A1.5 1.5 0 0 1 2.5 2h2.879a1.5 1.5 0 0 1 1.06.44L7.56 3.56A.5.5 0 0 0 7.914 3.75H13.5A1.5 1.5 0 0 1 15 5.25v7.25a1.5 1.5 0 0 1-1.5 1.5h-11A1.5 1.5 0 0 1 1 12.5v-9z"
      fill="#DCB67A"
    />
  </svg>
);
...
const FileIcon = () => (
  <svg width="16" height="16" viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg">
    <path
      d="M4 1.5A1.5 1.5 0 0 1 5.5 0h4.879a1.5 1.5 0 0 1 1.06.44l2.122 2.12a1.5 1.5 0 0 1 .439 1.061V14.5A1.5 1.5 0 0 1 12.5 16h-7A1.5 1.5 0 0 1 4 14.5v-13z"
      fill="#8DA3BF"
    />
  </svg>
);
...
const ParentDirIcon = () => (
  ...
  >
    <path
      fillRule="evenodd"
      d="M8 11.293l-5.146-5.147a.5.5 0 1 0-.708.708l5.5 5.5a.5.5 0 0 0 .708 0l5.5-5.5a.5.5 0 0 0-.708-.708L8 11.293z"
...
export const RemotePathPicker: React.FC<RemotePathPickerProps> = ({
  ...
  const handleInputChange = useCallback(
    (value: string) => {
      ...
      if (
        normalized &&
        (normalized === '~' || normalized.endsWith('\\') || normalized.endsWith('/'))
      ) {
  ...
  const filteredItems = useMemo(() => {
    ...
    if (searchTerm === pathPrefix || searchTerm.startsWith(pathPrefix)) {
      ...
      return items.filter((item) => item.name.toLowerCase().includes(filterPart));
    }
  ...
  return (
    <div className="remote-path-picker-overlay" onClick={() => onOpenChange(false)}>
      <div className="remote-path-picker-container" onClick={(e) => e.stopPropagation()}>
        <Command className="remote-path-picker-command" shouldFilter={false} loop>
          {/* 头部标题 */}
            ...
            {!loading && !error && (
              <>
                ...
                {currentPath && !loading && filteredItems.length === 0 && !error && (
                  <Command.Empty className="rpp-empty">此目录为空</Command.Empty>
                )}
          ...
          <div className="rpp-footer">
            ...
            <div className="rpp-footer-actions">
              {(mode === 'folder' || mode === 'both') && currentPath && (
                <button className="rpp-btn rpp-btn-primary" onClick={handleConfirmCurrentPath}>
                  选择此文件夹
                </button>
              )}
              <button className="rpp-btn" onClick={() => onOpenChange(false)}>
                取消
              </button>
            </div>
          </div>
