...
export const FileTree: React.FC<FileTreeProps> = ({
  ...
  return (
    <div className="flex-1 overflow-y-auto custom-scrollbar select-none py-1 bg-theme-surface">
      {tree && (
        <TreeNode
          key={tree.path}
          item={tree}
  ...
  return (
    <div className="flex flex-col">
      ...
      >
        ...
        {openedFile && (
          <div
            className="ml-2 w-1.5 h-1.5 rounded-full bg-green-500/50 shadow-[0_0_5px_rgba(34,197,94,0.3)] shrink-0"
            title="已打开"
          />
        )}
      </div>
...
const FileIcon: React.FC<{ name: string; isDir: boolean; isExpanded: boolean }> = ({
  name,
  isDir,
  isExpanded,
}) => {
  if (isDir) {
    const folderColor = 'text-[#858585]';
    let dot: React.ReactNode = null;

    if (name === '.agent') {
      dot = (
        <div className="absolute -right-0.5 -top-0.5 w-1.5 h-1.5 rounded-full bg-purple-500 border border-theme-surface" />
      );
    } else if (name === 'docs') {
      dot = (
        <div className="absolute -right-0.5 -top-0.5 w-1.5 h-1.5 rounded-full bg-blue-500 border border-theme-surface" />
      );
    } else if (name === 'node_modules') {
      dot = (
        <div className="absolute -right-0.5 -top-0.5 w-1.5 h-1.5 rounded-full bg-green-500 border border-theme-surface" />
      );
    } else if (name === 'tests') {
      dot = (
        <div className="absolute -right-0.5 -top-0.5 w-1.5 h-1.5 rounded-full bg-red-500 border border-theme-surface" />
      );
    }
  ...
  if (name === 'tsconfig.json')
    return <span className="text-[#3178c6] font-black text-[9px]">T⚙</span>;
  if (name === 'vite.config.ts')
    return <span className="text-[#bd34fe] font-black text-[10px]">⚡</span>;

  return (
    <div className="w-4 h-4 flex items-center justify-center">
      {iconMap[ext || ''] || iconMap['log']}
    </div>
  );
};
