    ...
    <div className="w-12 bg-theme-header flex flex-col items-center py-2 shrink-0 h-full border-r border-theme-subtle">
      {visibleIcons.map((item) => (
        <button
          ...
          className={`w-10 h-10 sm:w-12 sm:h-12 flex items-center justify-center relative group transition-colors ${
            activeView === item.id
              ? 'text-theme-primary'
      ...
      <div className="mt-auto space-y-1">
        ...
        {onToggleWatch && (
          ...
          >
            {isWatching && <div className="absolute left-0 w-0.5 h-full bg-green-400" />}
            <svg
              className="w-5 h-5 sm:w-6 sm:h-6"
              fill="none"
              stroke="currentColor"
