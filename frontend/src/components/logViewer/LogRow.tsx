...
function mergeHighlights(
  ...
  if (!highlights || highlights.length === 0) {
    return contentLength > 0
      ? [{ start: 0, end: contentLength, color: '', opacity: 100, isSearch: false }]
      : [];
  }
...
export const LogRow: React.FC<LogRowProps> = memo(
  ({
    ...
    const rowBackground = isHighlighted
      ...
      : layerResult.rowStyle?.backgroundColor ||
        (isMarked ? colors.BOOKMARK_BACKGROUND : 'transparent');
    ...
    return (
      <div className="log-row" data-log-index={index} style={rowStyleCSS}>
        {showLineNumbers && (
          <span
        ...
        {isLoaded ? (
          <span className="log-row-content" style={{ flex: 1, minWidth: 0, color: textColor }}>
            {segments.map((seg, i) => {
              ...
              if (seg.color) {
                const alphaHex = Math.max(
                  0,
                  Math.min(255, Math.floor(((seg.opacity ?? 100) / 100) * 255)),
                )
                  .toString(16)
                  .padStart(2, '0');
                return (
                  <span
                    key={i}
                    style={{
                      color: seg.color.startsWith('#') ? `${seg.color}${alphaHex}` : seg.color,
                    }}
                  >
                    {text}
                  </span>
          ...
          <span
            className="log-row-content log-row-skeleton"
