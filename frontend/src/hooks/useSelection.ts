...
export function useSelection({ lines, onSelectedTextChange }: UseSelectionOptions): UseSelectionReturn {
    ...
    const getLineSelectionRange = useCallback((lineIndex: number, norm: NormalizedSelection, contentLength: number): { s: number; e: number } => {
    ...
    const getSelectedText = useCallback((sel: Selection | null): string => {
        if (!sel) return '';
