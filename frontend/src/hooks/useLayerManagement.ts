...
export interface UseLayerManagementReturn {
  ...
  // 图层操作
  updateLayers: (
    updater: LogLayer[] | ((prev: LogLayer[]) => LogLayer[]),
    skipHistory?: boolean,
  ) => void;
  ...
  handleSavePresetWithName: (name: string) => void;
  saveStatus: 'idle' | 'saved';
  /** 应用预设（合并语义）：同 kind 同 name 的图层启用，缺失的追加；返回 { added, enabled } */
  applyPreset: (preset: LayerPreset) => { added: number; enabled: number };
}
...
export function useLayerManagement({
  ...
  const layersFunctionalHash = useMemo(() => {
    return JSON.stringify(layers.map((l) => [l.id, l.enabled, l.groupId, l.type, l.config]));
  }, [layers]);
  ...
  useEffect(() => {
    ...
    // 确保有一个系统默认预设
    let defaultPreset = initialPresets.find(
      (p) => p.id === DEFAULT_PRESET_ID || p.name === '默认' || p.name === 'Default',
    );
  ...
  const updateLayers = useCallback(
    (updater: LogLayer[] | ((prev: LogLayer[]) => LogLayer[]), skipHistory = false) => {
  ...
  const undo = useCallback(() => {
    ...
    setFiles((prev) =>
      prev.map((file) => {
        if (file.id !== activeFileId || !file.history || file.history.past.length === 0)
          return file;
  ...
  const redo = useCallback(() => {
    ...
    setFiles((prev) =>
      prev.map((file) => {
        if (file.id !== activeFileId || !file.history || file.history.future.length === 0)
          return file;
  ...
  const addLayer = useCallback(
    (type: LayerType | string, initialConfig: any = {}) => {
  ...
  /**
   * 保存当前的图层配置为预设（指定名称，不弹 prompt）。
   */
  const handleSavePresetWithName = useCallback(
    (presetName: string) => {
      ...
      setPresets((prev) => {
        ...
        const newPreset = {
          ...
          name: existingIdx >= 0 ? next[existingIdx].name : presetName.trim(),
  ...
  /**
   * 保存当前的图层配置为预设（prompt 输入名称）。
   */
  const handleSavePreset = useCallback(() => {
    const presetName = prompt("输入预设名称 (输入 '默认' 将更新系统设置):");
    if (!presetName) return;
    handleSavePresetWithName(presetName);
  }, [handleSavePresetWithName]);

  /**
   * 应用预设（合并语义）：同 kind 同 name 的图层启用，缺失的图层追加。
   * 返回 { added, enabled } 计数，供 UI 反馈。
   */
  const applyPreset = useCallback(
    (preset: LayerPreset): { added: number; enabled: number } => {
      let added = 0;
      let enabled = 0;
      ...
      if (newLayers.length > 0) {
        updateLayers((prev) => {
          const next = prev.map((el) => {
            const upd = newLayers.find((nl) => nl.id === el.id);
            return upd ? { ...el, ...upd } : el;
          });
          const toAdd = newLayers.filter((nl) => !prev.some((el) => el.id === nl.id));
          return [...next, ...toAdd];
        });
      }
