...
def test_multi_panel_search_independence(page, two_log_files, frontend_errors):
    ...
    marks_a = page.locator("mark").count()
...
def test_find_visible_per_panel_and_esc(page, two_log_files, frontend_errors):
    ...
    page.wait_for_selector(".log-row", timeout=60000)
    # B 的 find 初始隐藏
    assert page.locator('input[placeholder="查找"]').count() == 0, "面板 B find 不应自动打开"

    # B 打开 find 并搜索
    _search(page, "unique-token-beta")
    assert page.locator('input[placeholder="查找"]').count() == 1, "面板 B find 应打开"

    # 切回 A：find 应恢复为打开状态（A 上次 find 可见）
    _switch_tab(page, name_a)
    page.wait_for_selector('input[placeholder="查找"]', timeout=10000)
    assert page.locator('input[placeholder="查找"]').count() == 1, "切回 A 后 find 应恢复打开"

    # Esc 第一段：收起保留高亮
    page.keyboard.press("Escape")
    page.wait_for_selector('input[placeholder="查找"]', state="detached", timeout=5000)
    assert page.locator('input[placeholder="查找"]').count() == 0, "Esc 第一段应收起 find"
    assert page.locator("mark").count() >= 1, "Esc 第一段应保留高亮"
    ...
    assert page.locator("mark").count() == 0, "Esc 第二段应清空搜索"
